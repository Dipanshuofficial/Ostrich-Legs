...
import { Cpu, Server, Box } from "lucide-react";
...
export const ThrottleControl = ({
  ...
  const safeValue = Math.min(value, MAX_SAFE_THROTTLE_PERCENT);
  const percent = safeValue / 100;

  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newVal = parseInt(e.target.value);
    onChange(Math.min(newVal, MAX_SAFE_THROTTLE_PERCENT)); // hard clamp
  };
    ...
    <Card className="relative overflow-hidden group p-5 bg-surface-white">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-2xl font-black text-text-main">{safeValue}%</p>
        </div>
      ...
      <div className="text-[9px] text-amber-600 mt-2 text-center font-medium">
        Hard safety cap: {MAX_SAFE_THROTTLE_PERCENT}% (prevents device freeze)
      </div>
    </Card>
  );
};
